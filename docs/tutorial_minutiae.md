...
*  Unprintable characters (ascii values <32 or >=127) require 4 nibbles per character (this seems bad, but is necessary to allow for the 1 nibble space/newline and no string terminators).
...
Unprintable chars < 256 require 5 nibbles.

## Encodings

Nibbles uses "PHP" style handling of encodings, which is that everything is just raw bytes. You can use UTF-8 in your .nbl files, it will just read the raw bytes and when you print it, it is still just raw bytes. So you can have strings like "こんにちは" in your nibbles files and they will display properly when output (if your shell interprets output in the same encoding as your text editor and that encoding is something where the characters 0-127 are encoded as 0-127, UTF-8 satisfies this). The only caveat is that if you take the length of this string it will be larger than 5 (and similar misbehavior from other operations that index into a string).
